...
mute flag for the `mute byaan` keyword, and last-reply timestamp for the
cold-thread check. Feature is always on when the Slack integration is active.
"""
...
def upgrade() -> None:
    with op.batch_alter_table("slack_conversations") as batch_op:
        ...
        batch_op.add_column(sa.Column("last_bot_reply_at", sa.TIMESTAMP(), nullable=True))
    ...
    op.create_index(
        "ix_slack_conversations_last_activity",
        "slack_conversations",
        ["last_activity_at"],
        unique=False,
    )
...
def downgrade() -> None:
    op.drop_index("ix_slack_conversations_last_activity", table_name="slack_conversations")
    op.drop_index("ix_slack_conversations_owned_lookup", table_name="slack_conversations")

    with op.batch_alter_table("slack_conversations") as batch_op:
        batch_op.drop_column("last_bot_reply_at")
